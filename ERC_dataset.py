...
class Emory_loader(Dataset):
    def __init__(self, txt_file, dataclass):
        ...
        neg = ['Mad', 'Sad', 'Scared']
        neu = ['Neutral']
        emodict = {'Joyful': "joy", 'Mad': "mad", 'Peaceful': "peaceful", 'Powerful': "powerful", 'Neutral': "neutral",
                   'Sad': "sad", 'Scared': 'scared'}
...
class IEMOCAP_loader(Dataset):
    def __init__(self, txt_file, dataclass):
        ...
        neg = ['fru', 'sad']
        neu = ['neu']
        emodict = {'ang': "angry", 'exc': "excited", 'fru': "frustrated", 'hap': "happy", 'neu': "neutral",
                   'sad': "sad"}
        self.sentidict = {'positive': pos, 'negative': neg, 'neutral': neu}
...
class DD_loader(Dataset):
    def __init__(self, txt_file, dataclass):
        ...
        emodict = {'anger': "anger", 'disgust': "disgust", 'fear': "fear", 'happiness': "happy", 'neutral': "neutral",
                   'sadness': "sad", 'surprise': "surprise"}
        self.sentidict = {'positive': pos, 'negative': neg, 'neutral': neu}
        for i, data in enumerate(dataset):
            if data == '\n' and len(self.dialogs) > 0:
...
class new_loader(Dataset):
    def __init__(self, txt_file, dataclass):
        ...
        self.sentiSet = set()
        # {'anger', 'disgust', 'fear', 'happiness', 'neutral', 'sadness', 'surprise'}
        # emotions = ['', 'happiness', 'love', 'sorrow', 'fear', 'disgust', 'none']
        # emotions = ['disgust', 'fear', 'happiness', 'love', 'none', 'sorrow']
        pos = ['Happiness', 'Love']
        neg = ['Sorrow', 'Fear', 'Disgust']
        neu = ['None']
        emodict = {'Happiness': "Happiness", 'Love': "Love", 'Sorrow': "Sorrow", 'Fear': 'Fear', 'Disgust': "Disgust",
                   'None': "None"}
        self.sentidict = {'positive': pos, 'negative': neg, 'neutral': neu}
        for i, data in enumerate(dataset):
            if data == '\n' and len(self.dialogs) > 0:
            ...
            speaker = data.strip().split('\t')[0]
            utt = ' '.join(data.strip().split('\t')[1:-1])
            emo = data.strip().split('\t')[-1].replace(" ", '')
